<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@page import="java.util.ArrayList"%>
    <%@page import="com.bean.ProductBean"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/css/bootstrap.min.css" integrity="sha384-TX8t27EcRE3e/ihU7zmQxVncDAy5uIKz4rEkgIXeMed4M0jlfIDPvg6uqKI2xXr2" crossorigin="anonymous">
<!-- <script src="//ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js"></script>
<script src="//netdna.bootstrapcdn.com/bootstrap/3.1.1/js/bootstrap.min.js"></script>
<link rel="stylesheet" type="text/css" href="//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css">   -->
<title>Insert title here</title>
</head>
<body>
<style>
	.card {
  flex-direction: row;
}
</style>
<%
	ArrayList<ProductBean> products = (ArrayList<ProductBean>) request.getAttribute("products");
	//HttpSession hs = request.getSession(false);
	String name = (String)session.getAttribute("user");
%>




<div class="container-fluid">
<nav class="navbar navbar-expand-lg navbar-light bg-light navbar-fixed-top" >
 <!--  <a class="navbar-brand" href="ProductController">Clothify</a> -->
  <a class="navbar-brand" href="ProductController"><img src="data:image/jpeg;base64," height="80px" width="80px"></a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item active">
        <a class="nav-link" href="ProductController">Home |<span class="sr-only">(current)</span></a>
      </li>
      
      <li class="nav-item active">
        <a class="nav-link" href="ProductTypeController?productType=shirt">Shirts |<span class="sr-only">(current)</span></a>
      </li>
      
      <li class="nav-item active">
        <a class="nav-link" href="ProductTypeController?productType=t-shirt">T-Shirts |<span class="sr-only">(current)</span></a>
      </li>
      
      <li class="nav-item active">
        <a class="nav-link" href="ProductTypeController?productType=jeans">Jeans |<span class="sr-only">(current)</span></a>
      </li>
      
      <li class="nav-item active">
        <a class="nav-link" href="ProductTypeController?productType=hoodie">Hoodies |<span class="sr-only">(current)</span></a>
      </li>
      
      <li class="nav-item active">
        <a class="nav-link" href="ProductTypeController?productType=Women's Top Wear">Women's Top Wear |<span class="sr-only">(current)</span></a>
      </li>
      
       <li class="nav-item active">
        <a class="nav-link" href="ProductGenderController?gender=male">Mens Wear |<span class="sr-only">(current)</span></a>
      </li>
      
       <li class="nav-item active">
        <a class="nav-link" href="ProductGenderController?gender=female">Womens Wear <span class="sr-only">(current)</span></a>
      </li>
      
     
   
      <!-- <li class="nav-item dropdown ">
        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          Shop By Gender
        </a>
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          <a class="dropdown-item" href="ProductGenderController?gender=male">Male</a>
          <a class="dropdown-item" href="ProductGenderController?gender=female">Female</a>
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="#">Something else here</a>
        </div>
        </li> -->
      
    </ul>
     <form class="form-inline my-2 " action="ProductSearchController">
      <input class="form-control mr-sm-2 input-sm" type="search" placeholder="Search By Name" aria-label="Search" name="search">
      <button class="btn btn-outline-success my-2 my-sm-0 mx-2 btn-sm" type="submit">Search</button>
    </form>
    <!-- <form class="form-inline my-2 my-lg-0">
      <input class="form-control mr-sm-2" type="search" placeholder="Search by type" aria-label="Search">
      <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
    </form> -->
    
    <a href="CartController?productId=0"><button type="button" class="btn btn-success mx-auto btn-sm">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person" viewBox="0 0 16 16">
  <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4Zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10Z"></path>
</svg>
				<%=name==null?"My Profile":name %>
               
              </button></a> 
              
              <a href="ProfileController"><button type="button" class="btn btn-danger mx-3 btn-sm">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person" viewBox="0 0 16 16">
  <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4Zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10Z"></path>
</svg>
				Log Out
               
              </button></a>
              
  </div>
</nav>

<%
		if (products == null || products.size() == 0) {
%>

	........No Records Found......
<%
		}
		else{
				for(ProductBean p:products ){
%>
<div class="card text-center w-90  my-5 "  style="margin-top:20px;">
  <img src=<%=p.getImage()%> class="card-img-left rounded mx-auto d-block" alt="..."style="width: 20rem;">
  <div class="card-body" >
    <h5 class="card-header"><%=p.getName() %></h5>
    <p class="card-text">Description : <%=p.getDescription() %></p>
    <p class="card-text">Price : <%=p.getPrice() %></p>
    <p class="card-text">Type : <%=p.getType()%></p>
    <p class="card-text">Gender : <%=p.getGender() %></p>
    <a href="CartController?productId=<%=p.getProductId() %>" class="btn btn-primary">Add To Cart</a>
  </div>
</div>
<%
				}
		}
%>

<div class="container-fluid">
  <footer class="d-flex flex-wrap justify-content-between align-items-center py-3 my-4 border-top">
    <p class="col-md-4 mb-0 text-muted">© 2023 Clothify, Inc</p>

    <a href="#" class="col-md-4 d-flex align-items-center justify-content-center mb-3 mb-md-0 me-md-auto link-dark text-decoration-none">
      <!-- <svg class="bi me-2" width="40" height="32"><use xlink:href="#bootstrap"></use></svg> -->
      <img alt="clothify-image" height="80px" width="80px" src="data:image/jpeg;base64,">
    </a>

    <ul class="nav col-md-4 justify-content-end">
      <li class="nav-item"><a href="ProductController" class="nav-link px-2 text-muted">Home</a></li>
      <li class="nav-item"><a href="ProductGenderController?gender=male" class="nav-link px-2 text-muted">Mens</a></li>
      <li class="nav-item"><a href="ProductGenderController?gender=female" class="nav-link px-2 text-muted">Womens</a></li>
      
    </ul>
  </footer>
</div>





</div>

</body>
</html>